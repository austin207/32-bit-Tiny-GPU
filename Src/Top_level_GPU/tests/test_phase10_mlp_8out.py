...
async def test_mlp_8out(dut):
    ...
    path = os.path.join(base, "../../assembler/builds/bin/phase10_mlp_8out.axelbin")
    ...
    if kc is None:
        ...
        assert False, f"phase10 hung after {elapsed} cycles"

    all_pass = True
    for addr, exp in sorted(expected.items()):
        got = u32_to_signed(data_memory.get(addr, 0))
        status = "PASS" if got == exp else "FAIL"
        print(f"  y[{addr-12}] mem[{addr}] = {got:4d} expected {exp:4d} {status}")
        all_pass &= (got == exp)

    print(f"  kernel_cycles = {kc}")
    assert all_pass, "phase10 output mismatch"